...
public class Graph {
	...
	public int getComponentsCount() {
		IsConnected connected = new IsConnected(this);
		return connected.getConnectedComponentsCountIfAny();
	}
	...
	/**
	 * Important to return only a copy of the adjacency list. Running time for
	 * making this copy: For each vertex v, the number of adjacent vertices to v
	 * that need to be copied into a new list (matched with v in the copy of the
	 * map) is deg v. Also, each vertex is processed (cloned and the clone is added
	 * to its list); processing is O(1) Therefore, it is the sum over v in V of 1 +
	 * deg(v), which is O(n+m).
	 */
